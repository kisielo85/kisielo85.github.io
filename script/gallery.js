...
Image.prototype.load = function (url, id, is_auto = false) {
    ...
    this.onload = function () {
        ...
        if (is_auto) auto_view_tick(img_scroll_t)
    }
    ...
    xmlHTTP.onprogress = function (e) {
        ...
        // first loop, saving loading data to load_timer
        if (time_left == -1) {
            time_left = 1
            load_timer = [progress, Date.now()]
        }
        // showing the progress bar if there is more than a second left
        else {
    ...
    // image downloaded, saving to cache and displaying blob
